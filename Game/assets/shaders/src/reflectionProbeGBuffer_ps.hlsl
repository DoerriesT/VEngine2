...
struct PSOutput
{
	float4 colorRoughness : SV_Target0;
...
PSOutput main(PSInput input)
{
	...
	// albedo
	float3 albedo = unpackUnorm4x8(material.albedo).rgb;
	{
		if (material.albedoTextureHandle != 0)
		{
			float4 albedoAlpha = g_Textures[material.albedoTextureHandle].Sample(g_AnisoRepeatSampler, input.texCoord);
	...
	float3 N = normalize(input.normal);
	{
		if (material.normalTextureHandle != 0)
		{
			...
			tangentSpaceNormal.xy = g_Textures[material.normalTextureHandle].Sample(g_AnisoRepeatSampler, input.texCoord).xy * 2.0f - 1.0f;
			...
			N = tangentSpaceNormal.x * input.tangent.xyz + tangentSpaceNormal.y * bitangent + tangentSpaceNormal.z * N;
