...
void printDirectoryHierarchy(FileFindHandle h, char *resultPath)
{
	auto &vfs = VirtualFileSystem::get();
	while (resultPath[0])
	{
		int isDirectory = (int)vfs.findIsDirectory(h);
		int size = (int)(vfs.size(resultPath)) * (1 - isDirectory);

		printf("%s %i\n", resultPath, size);

		if (isDirectory)
		{
			char subDirPath[IFileSystem::k_maxPathLength];
			auto subDirHandle = vfs.findFirst(resultPath, subDirPath);

			printDirectoryHierarchy(subDirHandle, subDirPath);

			vfs.findClose(subDirHandle);
		}

		vfs.findNext(h, resultPath);
	}
}

int main(int argc, char *argv[])
{
	//const char *testPath1 = "i/start/without/separator.png";
	//const char *testPath2 = "/i/start/with/separator.png";
	//
	//printf("%s extension: %s\n", testPath1, Path::getExtension(testPath1));
	//printf("%s extension: %s\n", testPath2, Path::getExtension(testPath2));
	//printf("%s segments: %i\n", testPath1, (int)Path::getSegmentCount(testPath1));
	//printf("%s segments: %i\n", testPath2, (int)Path::getSegmentCount(testPath2));
	//
	//size_t count0 = Path::getSegmentCount(testPath1);
	//size_t count1 = Path::getSegmentCount(testPath2);
	//
	//assert(count0 == count1);
	//
	//for (size_t i = 0; i < count0; ++i)
	//{
	//	size_t offset1 = Path::getFirstNSegments(testPath1, i + 1);
	//	size_t offset2 = Path::getFirstNSegments(testPath2, i + 1);
	//	
	//	char tmp[256];
	//	
	//	//memcpy(tmp, testPath1, offset1);
	//	//tmp[offset1] = '\0';
	//	//printf("%i %s\n", (int)i, tmp);
	//	//
	//	//memcpy(tmp, testPath2, offset2);
	//	//tmp[offset2] = '\0';
	//	//printf("%i %s\n", (int)i, tmp);
	//
	//	size_t start = 0;
	//	size_t end = 0;
	//	
	//	Path::getSegmentDelimiters(testPath1, i, &start, &end);
	//	memcpy(tmp, testPath1 + start, end - start);
	//	tmp[end - start] = '\0';
	//	printf("%i %s\n", (int)i, tmp);
	//	
	//	Path::getSegmentDelimiters(testPath2, i, &start, &end);
	//	memcpy(tmp, testPath2 + start, end - start);
	//	tmp[end - start] = '\0';
	//	printf("%i %s\n", (int)i, tmp);
	//}
	//
	//auto &vfs = VirtualFileSystem::get();
	//vfs.mount("D:/Users/Tim/Documents/C++ Projects/VEngine2/Game/assets", "assets");
	//
	//char findPath[IFileSystem::k_maxPathLength];
	//auto h = vfs.findFirst("/assets", findPath);
	//
	//printDirectoryHierarchy(h, findPath);
	//
	////while (findPath[0])
	////{
	////	int isDirectory = (int)vfs.findIsDirectory(h);
	////	int size = (int)(vfs.size(findPath));
	////
	////	Log::info(findPath);
	////
	////	h = vfs.findNext(h, findPath);
	////}
	//
	//vfs.findClose(h);
